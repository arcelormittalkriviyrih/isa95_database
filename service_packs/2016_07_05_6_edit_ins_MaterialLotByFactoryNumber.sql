...
BEGIN
   ...
   FROM (SELECT ml.[ID],
         ...
         WHERE ml.[FactoryNumber]=@FACTORY_NUMBER) ml
   WHERE ml.RowNumber=1;

   IF @MaterialLotID IS NULL
      BEGIN
         SET @err_message = N'By FactoryNumber [' + @FACTORY_NUMBER + N'] MaterialLot not found';
         THROW 60010, @err_message, 1;
      END;

   EXEC [dbo].[ins_MaterialLot] @FactoryNumber = @FACTORY_NUMBER,
                                @Status        = N'1',
                                @Quantity      = @Quantity,
