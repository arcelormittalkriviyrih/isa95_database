...
BEGIN
   ...
   INSERT INTO [dbo].[OpMaterialRequirement] ([MaterialClassID],[MaterialDefinitionID],[MaterialLotID],[MaterialSubLotID])
   SELECT md.[MaterialClassID],md.[ID],ml.[ID] MaterialLotID, msl.[ID] MaterialSubLotID
   FROM [dbo].[MaterialDefinition] md 
        LEFT OUTER JOIN [dbo].[MaterialLot] ml ON (ml.[MaterialDefinitionID]=md.[ID])
        LEFT OUTER JOIN [dbo].[MaterialSubLot] msl ON (msl.[MaterialLotID]=ml.[ID])
   WHERE md.[ID]=@PROFILE;
   ...
   UNION ALL
   SELECT N'TMPL',@TEMPLATE WHERE @TEMPLATE IS NOT NULL;
