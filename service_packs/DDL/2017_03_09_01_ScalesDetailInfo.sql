     ...
     WITH BarWeight
          ...
          SELECT eq.ID AS ID,
				 ...
				 (SELECT 1 WHERE EXISTS (SELECT NULL FROM dbo.JobOrder jo WHERE jo.WorkType = N'KEPCommands'
																		   ...
																		   and DATEDIFF(second, jo.StartTime, CURRENT_TIMESTAMP) > 10 )
						  OR NOT EXISTS (SELECT * from dbo.kep_controller_diag diag where DATEPART(second, CURRENT_TIMESTAMP-diag.Controller_Last_Connect)<10 
