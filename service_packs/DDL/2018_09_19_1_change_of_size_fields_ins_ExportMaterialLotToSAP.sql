...
BEGIN
   ...
   --BEGIN TRY
	 ...
	 SET language Russian;
      ...
      SELECT (SELECT REPLICATE('0',12-LEN(t.[Value]))+t.[Value] FROM @tblProperty t WHERE t.[Description]=N'PROD_ORDER'), --AUFNR
             ...
             (SELECT cast(RTRIM(LTRIM(t.[Value])) as nvarchar(10)) FROM @tblProperty t WHERE [Description]=N'PART_NO'), --PARTY
             isnull((SELECT CAST(t.[Value] AS NUMERIC(10,0)) FROM @tblProperty t WHERE t.[Description]=N'BUNT_NO' AND ISNUMERIC(t.[Value])=1),0), --BUNT
              @Quantity, --MAS
             (SELECT cast(RTRIM(LTRIM(t.[Value])) as nvarchar(10)) FROM @tblProperty t WHERE t.[Description]=N'MELT_NO'), --PLAVK
              @Status, --OLD_BIR
             (SELECT t.[Value] FROM @tblProperty t WHERE t.[Description]=N'MILL_ID'), --AUART
              @FactoryNumber, --EO
              ISNULL((SELECT case SIGN(CAST(t.[Value] AS NUMERIC(10,0)))
							 when -1 then 0 else CAST(t.[Value] AS NUMERIC(10,0)) end
					  FROM @tblProperty t WHERE t.[Description]=N'LEAVE_NO' AND ISNUMERIC(t.[Value])=1),0), --N_ORDER,
             (SELECT CONVERT(DATETIME,t.[Value],104) FROM @tblProperty t WHERE t.[Description]=N'MEASURE_TIME' AND ISDATE(t.[Value])=1), --DT,
              ISNULL((CASE @Status
