...
BEGIN
   ...
   DECLARE @WorkRequestID INT, @WorkDefinitionID INT, @LABEL_PRINT_QTY [NVARCHAR](50);


   SET @WorkDefinitionID = dbo.get_EquipmentPropertyValue(@EquipmentID, N'STANDARD_WORK_DEFINITION_ID');

   SELECT @LABEL_PRINT_QTY=ps.[Value]
   FROM [dbo].[ParameterSpecification] ps
        INNER JOIN [dbo].[PropertyTypes] pt ON (pt.[ID]=ps.[PropertyType] AND pt.[Value]=N'LABEL_PRINT_QTY')
