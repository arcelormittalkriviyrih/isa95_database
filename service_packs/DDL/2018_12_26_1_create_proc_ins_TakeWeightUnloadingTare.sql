...
set 
	...
	,WO.[TaringPersonID] = TARE.[TaringPersonID]
...
(	-- first Tare after Brutting for each wagon
	select top 1 
		...
		,[TaringPersonID]
	from CTE_Tare CTE 
	where CTE.[ID] = PUD.[PackagingUnitsID] and CTE.[ValueTime] > WO.[OperationTime] 
	order by CTE.[ValueTime]
) as TARE
...
on PUD.[PackagingUnitsID] = PUP.[PackagingUnitsID]
where PUP.[Description] in (N'Грузоподъемность', N'Тара с бруса')
