...
/*
	Procedure: upd_SaveWaybill
	Изменяет свойства путевых
*/
...
BEGIN
	...
	insert into [dbo].[DocumentationsProperty]
		...
		,[ValueTime])	
	SELECT 
		 isnull(new.[Description], vDP.[Description]) as [Description]
		,new.[Value]
	...
	full join [dbo].[v_WGT_DocumentsProperty] vDP
	on vDP.[Description2] = new.[Description2]		
	inner join [dbo].[DocumentationsClassProperty] DCP
	on new.[Description] = DCP.[Description]
	where isnull(vDP.[value], '') !=isnull(new.[value], '') and [DocumentationsID] = @DocumentationsID
	...
	on D.ID = vDP.DocumentationsID
	where vDP.[Description2] = N'WaybillNumber' /*and vDP.[Value2] != @WaybillNumber*/ and D.[ID] = @DocumentationsID
