...
join (
	select
		 [ID]			as [WeightingOperationsID]
		,N'Вес тары'	as [Description]
		,[Tara]			as [Value]
		,[TaringTime]	as [ValueTime]
		,1				as [RN]
	from [dbo].[WeightingOperations]
	--where [OperationType] = N'Тарирование'
	union all
	select
		...
		,[Value]
		,[ValueTime]
		,row_number() over (partition by [WeightingOperationsID], [Description] order by [ValueTime] desc) [RN]
	from [dbo].[WeightingOperationsProperty]
) P
on P.[WeightingOperationsID] = WO.[ID] and [RN] = 1
where D.[ID]=@DocumentationsID 
	and D.[Status] = N'closed'
	and DC.[Description] = N'Тарирование' 
	and isnull(WO.[Status], '') != N'reject'
