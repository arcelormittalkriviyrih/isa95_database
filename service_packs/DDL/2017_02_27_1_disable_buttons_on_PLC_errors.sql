     ...
     WITH BarWeight
          ...
          SELECT eq.ID AS ID,
				 ...
				 (SELECT 1 WHERE EXISTS (SELECT NULL FROM dbo.JobOrder jo WHERE jo.WorkType = N'KEPCommands'
																		   ...
																		   and DATEDIFF(second, jo.StartTime, CURRENT_TIMESTAMP) > 30 )) Send_OPC_Error
...
SELECT ABS(CHECKSUM(NEWID())) AS ID,
	N'Нет связи с ПЛК для '+workshop.Description+N'\'+mill.Description+N'\'+side.[Description]
     ...
     WHERE p.ID = dbo.get_CurrentPerson()           
           AND (DATEPART(second, CURRENT_TIMESTAMP-diag.Controller_Last_Connect)>5 OR diag.Controller_Last_Connect IS NULL)
