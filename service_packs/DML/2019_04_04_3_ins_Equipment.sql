...
into #tmp_mat1
from (values (N'Горный департамент','Site',@ecid_site ,@equ_site)) as T([Description] ,[EquipmentLevel],[Equipment]  ,[EquipmentClassID])

-- drop table #tmp_mat1
-- insert Equipment-Site (Горный департамент)
...
set @ecid = (SELECT  top 1  [ID]  FROM [KRR-PA-ISA95_PRODUCTION].[dbo].[Equipment] where [Description] = N'Горный департамент' and [EquipmentLevel] = N'Site'); 
...
into #tmp_mat2
from (values
		 (N'Горный департамент (ГД)','Area',@ecid ,@equ)
		 ,(N'Горнотранспортный цех (ГТЦ)','Area',@ecid ,@equ)
		 ,(N'УЖДТ ГД','Area',@ecid ,@equ)
		 ,(N'Цех взрывных работ (ЦВР ГД)','Area',@ecid ,@equ)
		 ,(N'Дробильная фабрика (ДФ ГД)','Area',@ecid ,@equ)
		 ,(N'Рудообогатительная фабрика №1 (РОФ-1 ГД)','Area',@ecid ,@equ)
		 ,(N'Рудообогатительная фабрика №2 (РОФ-2 ГД)','Area',@ecid ,@equ)
		 ,(N'Цех технологического водоснабжения','Area',@ecid ,@equ)
		 ,(N'Цех сетей и подстанций','Area',@ecid ,@equ)
		 ,(N'Цех шламового хозяйства РОФ','Area',@ecid ,@equ)
		 ,(N'Цех испытательного центра','Area',@ecid ,@equ)
		 ,(N'Ремонтный цех','Area',@ecid ,@equ)
) as T([Description] ,[EquipmentLevel],[Equipment]  ,[EquipmentClassID])

-- drop table #tmp_mat2
-- insert Equipment-Area (Горный департамент)
	...
	-- insert property SAP_CODE for Equipment-Area (Горный департамент)
	...
	-- insert property CONSIGNER for Equipment-Area (Горный департамент)
		merge [dbo].[EquipmentProperty]  as trg 
		using
		(select distinct
				 MCP.[Description]				as [Description]
		...
		-- insert property CONSIGNEE for Equipment-Area (Горный департамент)
		merge [dbo].[EquipmentProperty]  as trg 
		using
		(select distinct
				 MCP.[Description]				as [Description]
