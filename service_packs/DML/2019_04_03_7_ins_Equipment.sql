...
set @ecid = (SELECT  top 1  [ID]  FROM [KRR-PA-ISA95_PRODUCTION].[dbo].[Equipment] where [Description] = N'Транспортный департамент' and [EquipmentLevel] = N'Site'); 
...
into #tmp_mat2
from (values
		 (N'Транспортный Департамент (ТД)','Area',@ecid ,@equ)
		 ,(N'Цех эксплуатации ж/д транспорта (ЦЭЖДТ)','Area',@ecid ,@equ)
		 ,(N'Цех ремонта подвижного состава (ЦРПС)','Area',@ecid ,@equ)
		 ,(N'ЦТА','Area',@ecid ,@equ)
		 ,(N'ЦПА ','Area',@ecid ,@equ)
) as T([Description] ,[EquipmentLevel],[Equipment]  ,[EquipmentClassID])

-- drop table #tmp_mat2
-- insert Equipment-Area (Транспортный департамент)
	...
	-- insert property SAP_CODE for Equipment-Area (Транспортный департамент)
	...
	-- insert property CONSIGNER for Equipment-Area (Транспортный департамент)
		merge [dbo].[EquipmentProperty]  as trg 
		using
		(select distinct
				 MCP.[Description]				as [Description]
		...
		-- insert property CONSIGNEE for Equipment-Area (Транспортный департамент)
		merge [dbo].[EquipmentProperty]  as trg 
		using
		(select distinct
				 MCP.[Description]				as [Description]
		...
		--insert Equipment-ProcessCell (Транспортный департамент)
		merge [dbo].[Equipment]  as trg 
		using
		(select  #tmp_mat2.[Description] , N'Process Cell' [EquipmentLevel] , @ecid2 [EquipmentClassID], eq.[id] [Equipment] from  #tmp_mat2 
			...
			where  #tmp_mat2.[Description] <> N'Транспортный Департамент (ТД)') as t2
		...
		-- insert property SAP_CODE for Equipment-ProcessCell (Транспортный департамент)
		merge [dbo].[EquipmentProperty]  as trg 
		using
		(select distinct
				 MCP.[Description]				as [Description]
			...
			join (select 	#tmp_mat2.[Description] , N'Process Cell' [EquipmentLevel] , @ecid2 [EquipmentClassID] , eq.[id] [Equipment]
						...
						join [KRR-PA-ISA95_PRODUCTION].[dbo].[Equipment] eq
						on #tmp_mat2.[Description] = eq.[Description]
						where eq.EquipmentClassID = @ecid2 and #tmp_mat2.[Description] <> N'Транспортный Департамент (ТД)') as T
			on MD.[Description] = T.[Description] 
			join [dbo].[EquipmentClassProperty] MCP
			on MCP.[EquipmentClassID] = MD.[EquipmentClassID]
