...
set @ecid = (SELECT  top 1  [ID]  FROM [KRR-PA-ISA95_PRODUCTION].[dbo].[Equipment] where [Description] = N'Транспортный Департамент (ТД)' and [EquipmentLevel] = N'Area'); 
...
into #tmp_mat2
from (values
			(N'Транспортный Департамент (ТД)',  N'Process Cell',  @ecid2, @ecid )
			,(N'Цех эксплуатации ж/д транспорта (ЦЭЖДТ)',  N'Process Cell',  @ecid2, @ecid )
			,(N'Цех ремонта подвижного состава (ЦРПС)',  N'Process Cell',  @ecid2, @ecid )
			,(N'ЦТА',  N'Process Cell',  @ecid2, @ecid )
			,(N'ЦПА',  N'Process Cell',  @ecid2, @ecid )
