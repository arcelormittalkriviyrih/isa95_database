...
for /f "tokens=*" %%a in (%WORKSPACE%\jenkins_scripts\all_files_scripts_sort.log) do (
	IF EXIST DDL\%%a (C:\Nikama\SysinternalsSuite\psexec.exe \\localhost -accepteula -f ^
					  -u %DDL_USER% -p %DDL_PASSWORD% -c "%WORKSPACE%\jenkins_scripts\apply_service_packs_all.bat" %WORKSPACE% krr-tst-pahwl02 ddl %%a || SET CMD_ERROR_LEVEL=1)
	IF EXIST DML\%%a (C:\Nikama\SysinternalsSuite\psexec.exe \\localhost -accepteula -f ^
					  -u %DML_USER% -p %DML_PASSWORD% -c "%WORKSPACE%\jenkins_scripts\apply_service_packs_all.bat" %WORKSPACE% krr-tst-pahwl02 dml %%a || SET CMD_ERROR_LEVEL=1)
)
